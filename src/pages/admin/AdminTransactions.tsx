  ...
  const approve = async (tx: any, action: 'approved' | 'rejected') => {
    ...
    try {
      if (action === 'rejected') {
        ...
        // If a withdrawal was already balance-deducted but OTP never sent,
        // refund the user. In normal flow admin rejects before approving,
        // so balance hasn't been touched yet.
        await api.transactions.update(tx.id, { status: 'rejected' });
        toast.success('Transaction rejected');

      } else if (tx.type === 'deposit') {
        // ── DEPOSIT APPROVAL ─────────────────────────────────────────────────
        // Credit user's balance and mark completed immediately (no OTP needed).
        ...
        // ── WITHDRAWAL APPROVAL ──────────────────────────────────────────────
        // Check balance → deduct it → generate OTP.
        // Balance is reserved now; user must verify OTP to complete.
        const current = await api.balances.getByEmail(tx.user_email);
        const currentBal = current?.balance_usd ?? 0;
        ...
        // ── SELL ORDER APPROVAL ──────────────────────────────────────────────
        // Remove crypto from portfolio → credit USD to balance → mark completed.
        if (tx.crypto_symbol && tx.crypto_amount && tx.crypto_amount > 0) {
